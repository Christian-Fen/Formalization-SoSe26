...
-- (Do not use any tactics!)
example :  a * (b * c) = (a * b) * c := Eq.symm (Nat.mul_assoc a b c)
...
example : (b + c) * a  = a * b + a * c:= Eq.trans (Nat.mul_comm (b + c) a) (Nat.left_distrib a b c)
