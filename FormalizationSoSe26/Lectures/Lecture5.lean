...
/-
Last time we discussed how to define sets and subsets in Lean.
We also saw how to define the basic operations on sets:
 - union,
 - intersection,
 - set difference.
We also saw how to define the empty set and the universal set.

Finally, we discussed functions: injective functions, surjective functions.
-/

variable (X Y : Type) (A B : Set X) (C : Set Y) (f : X → Y)
...
variable (A B C : Finset X)
/-
We need some assumption about classical logic to know that
a certain subset is finite.

See this example:
-/
-- #check A ∩ B
/-
One way is to explicitly state the logic is
`classical` and the function is `noncomputable`.
-/
open Classical in noncomputable example : Finset X := A ∩ B

/-
A more efficient way is to assume that  `X` has `decidable equality`.

This will in particular imply that finite subsets are closed under the relevant operations.
-/
...
We shall see later how `instance` operates in Lean.
-/

example : DecidableEq ℕ := by infer_instance

/-
This means for `ℕ` we can work directly with finite subsets.
-/
...
#check A.card
/-
This notation is very cumbersome, we want to use `#A` instead of `A.card`.

We can achieve that via `namespace`. We shall see more about namespaces soon.
...
--  `s \xs t` is the `Cartesian product` of `s` and `t`.
#check s ×ˢ t

example : #(s ×ˢ t) = #s * #t := by rw [card_product]
example : #(s ×ˢ t) = #s * #t := by simp only [card_product]

example : #(s ∪ t) = #s + #t - #(s ∩ t) := by rw [card_union]

example (h : Disjoint s t) : #(s ∪ t) = #s + #t := by rw [card_union_of_disjoint h]
example (h : Disjoint s t) : #(s ∪ t) = #s + #t := by simp [card_union_of_disjoint, h]
...
/-
Similarly, if we assume that `α` and `β` are `Fintype`, then
we have well-defined cardinality computations.
-/
...
/-
We can use induction to define many functions, which you can try in the exercises.
...
-- Note that `induction'` is discouraged these days.
...
-- Using `simp?` we can also prove it more easily.
example : ¬ (3 ∣ 5) := by
  simp only [Nat.reduceDvd, not_false_eq_true]
...
theorem two_le {m : ℕ} (h0 : m ≠ 0) (h1 : m ≠ 1) : 2 ≤ m := by
  ...
  · case succ m =>
      ...
      · apply Nat.succ_le_succ
        apply Nat.succ_le_succ
        -- We can use `repeat` to avoid writing the same line multiple times.
...
theorem exists_prime_factor {n : Nat} (h : 2 ≤ n) : ∃ p : Nat, p.Prime ∧ p ∣ n := by
  ...
  · induction n using Nat.strong_induction_on with
    ...
    | h n ih =>
        ...
        We already know that `m ≠ 1`, so we only need to show `m ≠ 0`.
        -/
        have : m ≠ 0 := by
          intro mz
          rw [mz, zero_dvd_iff] at mdvdn
          linarith
        -- Now it follows directly that `m ≥ 2`.
        have mgt2 : 2 ≤ m := two_le this mne1
        -- Finally, we can apply strong induction to `m`.
        by_cases mp : m.Prime
